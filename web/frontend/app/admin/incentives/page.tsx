...
function AdminIncentivesTable() {
  ...
  return (
    <div>
      <h1 className="text-xl font-bold mb-4">Admin Incentives</h1>
      <p className="text-sm text-muted mb-4">
        Full CRUD access.{" "}
        <Link href="/incentives" className="text-brand hover:underline">
          Read-only view
        </Link>{" "}
        for upcoming incentives.
      </p>
            ...
            <tbody>
              {filtered.map((x) => (
                <tr key={x.uuid} className="border-b border-border/50 hover:bg-surface/80">
                  ...
                  <EditableTextCell
                    value={x.incentive_text}
                    onSave={(val) => patchIncentive(x.uuid, { incentive_text: val })}
